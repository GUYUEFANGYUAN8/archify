...
| **交付** | 在目标同目录生成并检查候选；只有成品/凭据对提交、恢复日志完成收尾且交付锁释放后，才可选用 `--open` 打开这个确切成品。 |
...
`deliver --open` 适合一次性的本地交互交付。它默认关闭，并且只在验证成品/凭据对提交、恢复日志完成收尾且交付锁释放后执行；系统无法打开时，交付仍保持成功，JSON 只写 stdout，stderr 会给出可手动打开的绝对路径。
